...
export default function SalesTab({ sales = [], barbers, products, chemicalPromos, shareConfig, vouchers, onSaveSale }: SalesTabProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6" id="sales-register">
      ...
      <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // If custom input changes, clear preset so user knows it is manual
                    setSelectedChemicalPromoId('');
                  }}
                  className="w-full pl-8 pr-3 py-2 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-all font-mono"
                />
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      if (promoId) {
                        ...
                        setSelectedChemicalPromoId('');
                        // Don't auto-clear chemicalInput in case they want to modify manually
                      }
                    }}
        ...
        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-4">
          ...
          <div className={`grid grid-cols-1 ${showPromoDiscount ? 'sm:grid-cols-2' : ''} gap-4 pt-1`}>
            {/* Promotion Checkbox */}
            {showPromoDiscount && (
              <label className={`flex items-start space-x-3 p-3 bg-white border rounded-xl cursor-pointer transition-all ${
                ...
                <input
                  ...
                  className="mt-1 w-4.5 h-4.5 text-emerald-600 bg-gray-100 rounded border-gray-300 focus:ring-emerald-500 outline-none"
                />
                <div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl appearance-none bg-white outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-xs"
                >
        ...
        <div className="flex space-x-3 pt-2">
          <button
            type="button"
            onClick={handleReset}
            className="w-1/3 py-3 border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold rounded-xl text-sm transition-all"
          >
            ล้างข้อมูล
          </button>
          <button
            ...
            className={`w-2/3 py-3 font-semibold rounded-xl text-sm transition-all text-white flex items-center justify-center space-x-2 ${
